...
import { loadConfig, loadSessionState, saveSessionState, setConfigValue, configPath } from "../core/state.ts";
...
async function chatMode(): Promise<void> {
  ...
  while (true) {
    const line = (await repl.question("> ")).trim();
    if (!line) {
      continue;
    }
    ...
    const result = await agent.run(line, state);
    state = result.state;
...
async function createAgent(persist = true): Promise<GenesisAgent> {
...
async function handleConfig(args: string[]): Promise<void> {
  ...
  if (subcommand === "show") {
    const config = await loadConfig();
    const redacted = { ...config, api_key: config.api_key ? "******" : "" };
  ...
  if (subcommand === "set") {
    ...
    console.log(JSON.stringify({ ...config, api_key: config.api_key ? "******" : "" }, null, 2));
    return;
  }
...
用法：
  ...
  genesis config set base_url <OpenAI兼容地址>
  genesis config set model <模型名>

说明：
  默认 provider 是 local，可离线验证 agent loop。
  真实模型可配置为 openai、anthropic 或 openai-compatible。
  所有面向用户的默认输出使用中文。
`);
}
